...
function mergeWorkspaceForSave(currentWorkspace, incomingWorkspace) {
  ...
  normalizeRoadmapRecords(current);
  normalizeRoadmapRecords(merged);
  normalizeContentRecords(current);
  normalizeContentRecords(merged);
  normalizeBoardRecords(current);
  normalizeBoardRecords(merged);
  normalizeEmployeeRecords(current);
  normalizeEmployeeRecords(merged);

  const currentRoadmapItems = current.sections?.roadmap?.months;
  const incomingRoadmapItems = merged.sections?.roadmap?.months;
  if (Array.isArray(currentRoadmapItems) && Array.isArray(incomingRoadmapItems)) {
    const deletedMonthIds = collectDeletedIds(current.sections?.roadmap?.deletedMonthIds, merged.sections.roadmap.deletedMonthIds, 1000);
    merged.sections.roadmap.deletedMonthIds = Array.from(deletedMonthIds).slice(-1000);
    merged.sections.roadmap.months = mergeTimestampedItems(currentRoadmapItems, incomingRoadmapItems, ROADMAP_ITEM_FIELDS, deletedMonthIds, roadmapItemMergeKey, compareRoadmapItems);
  }

  const currentContentWeeks = current.sections?.content?.weeks;
  const incomingContentWeeks = merged.sections?.content?.weeks;
  if (Array.isArray(currentContentWeeks) && Array.isArray(incomingContentWeeks)) {
    const deletedWeekIds = collectDeletedIds(current.sections?.content?.deletedWeekIds, merged.sections.content.deletedWeekIds, 500);
    const deletedItemIds = collectDeletedIds(current.sections?.content?.deletedItemIds, merged.sections.content.deletedItemIds, 1000);
    merged.sections.content.deletedWeekIds = Array.from(deletedWeekIds).slice(-500);
    merged.sections.content.deletedItemIds = Array.from(deletedItemIds).slice(-1000);
    merged.sections.content.weeks = mergeContentWeeks(currentContentWeeks, incomingContentWeeks, deletedWeekIds, deletedItemIds);
  }

  const currentProjects = current.sections?.board?.projects;
  const incomingProjects = merged.sections?.board?.projects;
  if (Array.isArray(currentProjects) && Array.isArray(incomingProjects)) {
    const deletedProjectIds = collectDeletedIds(current.sections?.board?.deletedProjectIds, merged.sections.board.deletedProjectIds, 1000);
    merged.sections.board.deletedProjectIds = Array.from(deletedProjectIds).slice(-1000);
    merged.sections.board.projects = mergeTimestampedItems(currentProjects, incomingProjects, BOARD_PROJECT_FIELDS, deletedProjectIds, projectMergeKey, compareProjectsByDate);
  }
...
function mergeTimestampedItems(currentItems = [], incomingItems = [], fields = [], deletedIds = new Set(), keyFn = itemMergeKey, compareFn = compareItemsByUpdatedAt) {
  const items = new Map();

  (Array.isArray(currentItems) ? currentItems : []).forEach((item, index) => {
    if (item?.id && deletedIds.has(String(item.id))) return;
    items.set(keyFn(item, index), item);
  });

  (Array.isArray(incomingItems) ? incomingItems : []).forEach((incomingItem, index) => {
    if (incomingItem?.id && deletedIds.has(String(incomingItem.id))) return;
    const key = keyFn(incomingItem, index);
    const currentItem = items.get(key);
    items.set(key, mergeTimestampedItem(currentItem, incomingItem, fields));
  });

  return Array.from(items.values()).sort(compareFn);
}

function mergeTimestampedItem(currentItem, incomingItem, fields = []) {
  ...
  const fieldUpdatedAt = {
    ...(currentItem.fieldUpdatedAt || {}),
    ...(incomingItem.fieldUpdatedAt || {})
  };

  for (const field of fields) {
    const currentFieldTime = currentItem.fieldUpdatedAt?.[field] || currentItem.updatedAt;
    const incomingFieldTime = incomingItem.fieldUpdatedAt?.[field] || incomingItem.updatedAt;
    if (isNewerTimestamp(currentFieldTime, incomingFieldTime)) {
      merged[field] = currentItem[field];
      fieldUpdatedAt[field] = currentFieldTime;
      continue;
    }

    fieldUpdatedAt[field] = incomingFieldTime || currentFieldTime || merged.updatedAt || now();
  }

  for (const field of fields) {
    const currentValue = String(currentItem[field] || "");
    const incomingValue = String(incomingItem[field] || "");
    if (currentValue.trim() && !incomingValue.trim()) {
      merged[field] = currentItem[field];
      fieldUpdatedAt[field] = currentItem.fieldUpdatedAt?.[field] || currentItem.updatedAt || fieldUpdatedAt[field];
    }
  }

  merged.fieldUpdatedAt = normalizeItemFieldTimestamps({ ...merged, fieldUpdatedAt }, fields);
  merged.updatedAt = maxTimestamp(currentItem.updatedAt, incomingItem.updatedAt, ...Object.values(merged.fieldUpdatedAt));
  return merged;
}

function mergeContentWeeks(currentWeeks = [], incomingWeeks = [], deletedWeekIds = new Set(), deletedItemIds = new Set()) {
  const weeks = new Map();

  (Array.isArray(currentWeeks) ? currentWeeks : []).forEach((week, index) => {
    if (week?.id && deletedWeekIds.has(String(week.id))) return;
    weeks.set(contentWeekMergeKey(week, index), week);
  });

  (Array.isArray(incomingWeeks) ? incomingWeeks : []).forEach((incomingWeek, index) => {
    if (incomingWeek?.id && deletedWeekIds.has(String(incomingWeek.id))) return;
    const key = contentWeekMergeKey(incomingWeek, index);
    const currentWeek = weeks.get(key);
    const mergedWeek = mergeTimestampedItem(currentWeek, incomingWeek, CONTENT_WEEK_FIELDS);
    mergedWeek.items = mergeTimestampedItems(
      currentWeek?.items || [],
      incomingWeek?.items || [],
      CONTENT_ITEM_FIELDS,
      deletedItemIds,
    ...
    weeks.set(key, mergedWeek);
  });

  return Array.from(weeks.values()).sort(compareContentWeeks);
}
...
function mergeEmployees(currentPeople, incomingPeople, isOlderSave, deletedEmployeeIds = new Set(), deletedReportIds = new Set()) {
  ...
  const mergedPeople = incomingPeople
    ...
    .map((incomingPerson, incomingIndex) => {
      ...
      const currentPerson = currentMatch.person;
      const keepCurrentPerson = isNewerTimestamp(currentPerson?.updatedAt, incomingPerson?.updatedAt);
      return {
        ...(keepCurrentPerson ? incomingPerson : currentPerson),
        ...(keepCurrentPerson ? currentPerson : incomingPerson),
...
function mergeReport(currentReport, incomingReport, isOlderSave) {
  ...
  const fieldUpdatedAt = {
    ...(currentReport.fieldUpdatedAt || {}),
    ...(incomingReport.fieldUpdatedAt || {})
  };

  for (const field of ["title", "text", "date", "status"]) {
    const currentFieldTime = currentReport.fieldUpdatedAt?.[field] || currentReport.updatedAt;
    const incomingFieldTime = incomingReport.fieldUpdatedAt?.[field] || incomingReport.updatedAt;
    if (isNewerTimestamp(currentFieldTime, incomingFieldTime)) {
      merged[field] = currentReport[field];
      fieldUpdatedAt[field] = currentFieldTime;
      if (field === "status") merged.completed = currentReport.completed;
      continue;
    }
  ...
  for (const field of ["title", "text"]) {
    ...
    if (currentValue.trim() && !incomingValue.trim()) {
      merged[field] = currentReport[field];
      fieldUpdatedAt[field] = currentReport.fieldUpdatedAt?.[field] || currentReport.updatedAt || fieldUpdatedAt[field];
    }
  }
  ...
  merged.fieldUpdatedAt = normalizeReportFieldTimestamps({ ...merged, fieldUpdatedAt });
  merged.updatedAt = maxTimestamp(currentReport.updatedAt, incomingReport.updatedAt, ...Object.values(merged.fieldUpdatedAt));
  merged.completed = merged.status === "done";
  return merged;
}

function normalizeRoadmapRecords(workspace) {
  const roadmap = workspace?.sections?.roadmap;
  if (!roadmap) return;

  const fallbackTime = roadmap.updatedAt || workspace.updatedAt || now();
  roadmap.deletedMonthIds = Array.isArray(roadmap.deletedMonthIds) ? roadmap.deletedMonthIds.filter(Boolean).slice(-1000) : [];
  roadmap.months = Array.isArray(roadmap.months) ? roadmap.months : [];
  roadmap.months.forEach((item, index) => {
    if (!item || typeof item !== "object") return;
    if (!item.id) item.id = `roadmap-${index}-${normalizeName(item.task || item.month) || "item"}`;
    if (!item.updatedAt) item.updatedAt = roadmap.updatedAt || fallbackTime;
    item.fieldUpdatedAt = normalizeItemFieldTimestamps(item, ROADMAP_ITEM_FIELDS);
  });
}

function normalizeContentRecords(workspace) {
  const content = workspace?.sections?.content;
  if (!content) return;

  const fallbackTime = content.updatedAt || workspace.updatedAt || now();
  ...
  content.weeks = Array.isArray(content.weeks) ? content.weeks : [];
  content.weeks.forEach((week, weekIndex) => {
    if (!week || typeof week !== "object") return;
    if (!week.id) week.id = `content-week-${weekIndex}-${normalizeName(week.title) || "week"}`;
    if (!week.updatedAt) week.updatedAt = content.updatedAt || fallbackTime;
    ...
    week.items.forEach((item, itemIndex) => {
      if (!item || typeof item !== "object") return;
      if (!item.id) item.id = `content-item-${week.id}-${itemIndex}-${normalizeName(item.title) || "item"}`;
      if (!item.updatedAt) item.updatedAt = week.updatedAt || content.updatedAt || fallbackTime;
...
function normalizeBoardRecords(workspace) {
  const board = workspace?.sections?.board;
  if (!board) return;

  const fallbackTime = board.updatedAt || workspace.updatedAt || now();
  ...
  board.projects.forEach((project, index) => {
    ...
    if (!project.id) project.id = `project-${index}-${normalizeName(project.title) || "item"}`;
    if (!project.updatedAt) project.updatedAt = board.updatedAt || fallbackTime;
...
function normalizeEmployeeRecords(workspace) {
  ...
  const fallbackTime = employees.updatedAt || workspace.updatedAt || now();
  employees.people.forEach((person, personIndex) => {
    if (!person || typeof person !== "object") return;
    if (!person.updatedAt) person.updatedAt = fallbackTime;
    ...
    person.reports.forEach((report, reportIndex) => {
      ...
      if (!report.updatedAt) report.updatedAt = person.updatedAt || fallbackTime;
...
function normalizeItemFieldTimestamps(item, fields) {
  const fallback = item.updatedAt || now();
...
function itemMergeKey(item, index) {
  ...
  return `item:${normalizeName(item?.title || item?.task || item?.month)}:${index}`;
}

function roadmapItemMergeKey(item, index) {
  if (item?.id) return `id:${item.id}`;
  return `roadmap:${item?.date || ""}:${normalizeName(item?.task || item?.month)}:${index}`;
}

function contentWeekMergeKey(week, index) {
  if (week?.id) return `id:${week.id}`;
  return `content-week:${normalizeName(week?.title)}:${index}`;
}

function contentItemMergeKey(item, index) {
  if (item?.id) return `id:${item.id}`;
  return `content-item:${item?.date || ""}:${normalizeName(item?.title)}:${index}`;
}

function projectMergeKey(project, index) {
  if (project?.id) return `id:${project.id}`;
  return `project:${project?.due || ""}:${normalizeName(project?.title)}:${index}`;
}
...
function normalizeContentTypes(item) {
  ...
  return [...new Set(source.map(type => String(type || "").trim()).filter(Boolean))];
}
...
function compareRoadmapItems(left, right) {
  return compareDateFields(left?.date, right?.date)
    || String(left?.task || left?.month || "").localeCompare(String(right?.task || right?.month || ""), "ru");
}

function compareContentWeeks(left, right) {
  return String(left?.title || "").localeCompare(String(right?.title || ""), "ru", { numeric: true });
}

function compareContentItems(left, right) {
  return compareDateFields(left?.date, right?.date)
    || String(left?.title || "").localeCompare(String(right?.title || ""), "ru");
}

function compareProjectsByDate(left, right) {
  return compareDateFields(left?.due, right?.due)
    || String(left?.title || "").localeCompare(String(right?.title || ""), "ru");
}

function compareItemsByUpdatedAt(left, right) {
  return compareDateFields(left?.updatedAt, right?.updatedAt);
}

function compareDateFields(left, right) {
