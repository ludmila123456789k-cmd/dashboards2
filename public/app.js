...
const BOARD_PROJECT_FIELDS = ["title", "owner", "stage", "due"];
...
const DEFAULT_WORKSPACE = {
  ...
  sections: {
    ...
    board: {
      ...
      stages: BOARD_STAGES,
      projects: [
        { id: "p1", title: "Новый поиск", owner: "Сайт", stage: "В работе", due: "2026-07-12" },
        { id: "p2", title: "Новый каталог", owner: "Контент", stage: "Идеи", due: "2026-08-01" },
        { id: "p3", title: "Новый фильтр", owner: "Сайт", stage: "Готово", due: "2026-06-20" },
  ...
  return `
    ...
    <div class="board">
      ${BOARD_STAGES.map(stage => {
        const projects = monthProjects.filter(project => project.stage === stage);
        return `
...
function setProjectStage(projectIdOrIndex, stage) {
  ...
  if (!project) return;
  project.stage = normalizeBoardStage(stage);
  touchProjectRecord(project, "stage");
  touchAndRender("board");
}
...
function boardProjectsForMonth(monthValue) {
  return (workspace.sections.board.projects || [])
    ...
    .map(project => ({ ...project, stage: normalizeBoardStage(project.stage) }));
}
