...
function editReport(reportId) {
  if (!reportId) return;
  editingReportIds.add(String(reportId));
...
function renderReport(report, personIndex) {
  ...
  const isEditing = !isPublicView && editingReportIds.has(reportId);
          ...
          : `<div class="report-summary-actions">
              ${isEditing
                ? `<span class="report-status-pill ${status}">${escapeHtml(reportStatusName(status))}</span>
                  <button class="button small" type="button" data-action="finish-report-edit" data-report-id="${escapeAttribute(report.id)}">Готово</button>`
                : `<span class="report-status-pill ${status}">${escapeHtml(reportStatusName(status))}</span>
        ...
        ${isEditing
          ? `
            <div class="report-edit-grid">
              ...
              <textarea class="textarea report-textarea" data-change="report-field" data-live-change="report-field" data-person-id="${escapeAttribute(personId)}" data-report-id="${escapeAttribute(report.id)}" data-field="text" placeholder="Описание задачи">${escapeHtml(report.text || "")}</textarea>
            </div>
            ${actions}
          `
          : `
...
function renderReportDescription(text) {
  ...
  return `<div class="report-description">${escapeHtml(value).replace(/\r?\n/g, "<br>")}</div>`;
}
...
async function handleAction(button) {
  ...
  if (action === "edit-report") {
    editReport(button.dataset.reportId);
    return;
  }
...
function actionChangesWorkspace(action) {
  return /^(add|remove|set|toggle)-/.test(action)
    || ["demo", "reset", "prev-day", "next-day", "prev-month", "next-month", "prev-calendar-month", "next-calendar-month"].includes(action);
}
