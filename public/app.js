  ...
  return `
    <div class="employee-workspace">
      ...
      <div class="panel employee-detail-panel">
        ...
        ${renderEmployeeTaskFinder(people, selectedMonth)}
...
function renderEmployeeTaskFinder(people, month) {
  const employees = workspace.sections.employees || {};
  const filter = employeeTaskFilterValue(employees.taskFilter);
  const items = employeeTaskFinderItems(people, month, filter);
...
function employeeTaskFinderItems(people, month, filter) {
...
function reportWorkQuantitySummary(reports) {
  ...
  reportsToCount.forEach(report => {
    const quantity = extractWorkQuantity(report.text);
    if (!quantity) return;
...
function extractWorkQuantity(text) {
  const match = String(text || "").match(/(?:^|[^\d])(\d+(?:[.,]\d+)?)\s*(шт\.?|штук|ед\.?|единиц[а-я]*|раз[а-я]*)/i);
