...
async function handleAction(button) {
  ...
  if (action === "remove-roadmap") removeRoadmapItem(Number(button.dataset.index));
  ...
  if (action === "remove-project") removeProject(Number(button.dataset.index));
...
function handleChange(input) {
  ...
  if (change === "project-due") {
    const board = workspace.sections.board;
    const project = board.projects[Number(input.dataset.index)];
    if (project) {
...
function removeContentItem(weekIndex, itemIndex) {
  ...
  const [removed] = content.weeks[weekIndex].items.splice(itemIndex, 1);
...
function setProjectStage(index, stage) {
  const project = workspace.sections.board.projects[index];
  if (!project) return;
