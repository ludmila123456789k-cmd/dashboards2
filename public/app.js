...
function renderEmployeeMonthlySummary(person, reports, month) {
  ...
  const reportTitles = uniqueReportTitles(reports).join(", ") || "Нет задач за месяц";
  ...
  return `
    <details class="monthly-summary">
      ...
      <div class="monthly-summary-body">
        ...
        <div class="monthly-work-counts">
          <strong>Количество сделанных работ:</strong>
          ${workQuantities.length
            ? workQuantities.map(item => `<div>${escapeHtml(item.title)} — ${escapeHtml(item.value)}</div>`).join("")
            : `<div>не указано</div>`}
        </div>
      </div>
    </details>
